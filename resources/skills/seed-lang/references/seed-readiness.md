...
1. Inspect `requirements`. Convert every raw entry into appropriate addressable
   sections and verifications, create and attach missing artifacts, then remove
   the converted entry. The Seed is not ready while any requirement remains.
2. Run `seed validate`, then read the complete `seed blueprint`, including
   genomes and global policies.
3. For every in-scope interface, behavior, state, error, security,
...
4. Confirm each required artifact is declared, exists at its repository-relative
   ...
   contract.
5. Reverse-check terms such as schema, payload, field, database, table, event,
   ...
   match.
6. Report **Implementation-ready** only when validation passes, `requirements`
   is empty, artifacts are
